...
|---|---|---|---|---|---|
| 1 | 道 | 問題定位 | 缺失值本身可能就是訊號。先講「為什麼缺」再講「怎麼補」，AI 可以寫插補程式，人要判斷缺失機制 | `01_missing_data_overview.ipynb` | 圖 1 |
...
| 6 | 法 | 驗收三問 | 人話、洩漏、拿掉會怎樣。離群值多一問：這是錯誤還是真實的極端值，兩者處理方式相反 | — | |
| 7 | 法 | 反例 | 用全資料算中位數再切分，測試分數虛高。附錯誤程式與前後分數 | `03_outlier_detection.ipynb` | 圖 3 |
| 8 | 器 | 工具速查 | `SimpleImputer`、`KNNImputer`、`IterativeImputer` 的最小可跑範例，含 `add_indicator` | `02_imputation_methods.ipynb` | |
| 9 | 器 | 三個陷阱與 notebook 對照 | 陷阱：對測試集重新 `fit`、把 MNAR 當 MCAR 補、用平均值補嚴重偏態欄位 | 全部 | |

要學員記住的那句話：缺失值不是要填滿的空格，是要先解釋的現象。
