...
|---|---|---|---|---|---|
| 1 | 道 | 四模態的共同處理骨架 | 文字圖像音訊影片四種模態，處理骨架都是理解、清理、導入張量 | — | 圖 1 |
| 2 | 道 | 四模態總覽 | 每種模態的原始形式、目標張量、常用模型，一頁看完 | `05_multimodal/01_image_text_pairs.ipynb` | 圖 2 |
...
| 8 | 法 | 反例：訓練語料污染 | 訓練語料裡含測試題目，分數虛高但上線就崩 | `01_text_features/04_llm_data_formats.ipynb` | 圖 4 |
| 9 | 器 | 文字工具速查 | tokenizer 的 padding、truncation、`input_ids` 與 `attention_mask` | `01_text_features/02_tokenization.ipynb` | |
| 10 | 器 | 影像工具速查 | `AutoImageProcessor`、ViT 與 CLIP 的輸入差異 | `02_image_features/03_modern_image_representations.ipynb` | |
| 11 | 器 | 音訊工具速查 | `torchaudio` 載入、重取樣、Whisper 與 wav2vec2 的輸入 | `03_audio_features/03_modern_audio_representations.ipynb` | |
| 12 | 器 | 影片工具速查 | 解碼、影格取樣、VideoMAE 的輸入形狀 | `04_video_features/01_video_to_tensor.ipynb` | |
