...
AI 可以 fit XGBoost。人必須把重要性對回 Why；`customerID` 編成數字若變很重要，那是無意義編碼。

**留下的那句話：** 對不回 Why 的高分特徵，先當可疑。示範用真實 Telco／Mall，不用 `make_classification`。

> 樹重要性與端到端是主線。關聯規則只上一小段，對照現代推薦。複本／Instacart CSV 在 `modules/extension/`。
...
- **基礎 (Fundamentals)**:
  1.  **關聯規則（一小段）**: 可解釋的共購報表，不是現代推薦主線。
  2.  **聚類**: 幾何直覺一張圖，落地用真實客戶資料。
  3.  **樹模型特徵重要性**: 真實 Telco；XGB 與 LightGBM 對照各一小段。
  4.  **端到端**: 真實資料 + sklearn Pipeline，先切再轉。

---

## 2. 關聯規則探勘（2026 怎麼放）

**事實：** 購物籃規則（Apriori／較常見的 FP-Growth）在零售 **BI、共購報表、可解釋搭售** 還看得到。它**不是**現代推薦／特徵平台的主路徑。業界主線是行為事件特徵、item／user embedding、排序模型。

| | 關聯規則 | 2026 推薦主線 |
|---|---|---|
| 產出 | 「買 A 也常買 B」（lift） | 對這個人現在排序什麼 |
| 優點 | 能講人話、好做報表 | 能個人化、吃稀疏互動 |
| 代價 | 項一多就爆；規則不等於會買 | 較難口頭解釋每一維 |

上課約一小時：講完指標，有 Instacart 真實檔就跑一小段（`02_instacart_case`，選做）。`01_apriori_algorithm` 的假購物籃只當演算法示意，可跳過。
...
> 合成月牙／圓形最多當「為什麼要 DBSCAN」的一張圖。落地走真實 `Mall_Customers.csv`（`03_mall_customers_case`）。
...
上課：**一張對照 + 各跑一小段 API**。完整流失案例只走 `notebooks/03_tree_models/03_telco_churn_case.ipynb`。

| | XGBoost | LightGBM |
|---|---|---|
| 常見重要性 | `weight` / `gain` / `cover` | `split` / `gain` |
| 這堂用它 | 看增益是否對得回特徵故事 | 同上，順便比速度與切樹方式 |
| 不上 | 調參大戰 | 調參大戰 |
