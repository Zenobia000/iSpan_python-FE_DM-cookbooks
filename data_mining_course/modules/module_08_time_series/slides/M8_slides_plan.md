...
講義本體 `M8_Time_Series_Feature_Engineering.md`，產圖提示詞在 [`M8_slides.prompts.md`](M8_slides.prompts.md)，圖檔輸出到 `concept_images/`。

落在六步的第 5 到 6 步。這堂對應的鐵律是鐵律一的時序版（按時間切，不准洗牌）。

| # | 格 | 標題 | 這張要呈現什麼 | 對應 notebook | 圖 |
|---|---|---|---|---|---|
| 1 | 道 | 這堂在解什麼 | 時間序列把鐵律一換了一種形式：切分單位是時間，測試集必須是比較晚的一段 | `01_lag_features.ipynb` | 圖 1 |
| 2 | 術 | 第一原理 | 窗只能看過去。任何用到當下或未來資料的窗，都會讓模型在真實情境失效 | `01_lag_features.ipynb` | |
| 3 | 術 | 決策表 | lag／rolling／expanding／季節分解四類，比看多久以前、參數、會不會不小心看到未來 | `02_rolling_windows.ipynb` | 圖 2 |
...
| 8 | 器 | API 速查 | `shift`、`rolling`、`expanding`、`TimeSeriesSplit`、`seasonal_decompose` 的最小範例 | `04_seasonality_trend.ipynb` | |
