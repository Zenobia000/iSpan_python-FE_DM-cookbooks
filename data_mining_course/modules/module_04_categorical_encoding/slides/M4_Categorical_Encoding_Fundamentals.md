...
AI 很會寫 One-hot。人必須決定類別在 Why 裡的角色，以及 Target encoding 有沒有漏進測試集。

**留下的那句話：** Target／Count 的統計只能看訓練摺。
...
四法各有一本技法 notebook；**同一份 Titanic 走完多種編碼只在 `05_titanic_case`。**

| 方法 | 適用 | 代價 | 洩漏 |
|---|---|---|---|
| Label | 樹模型、有序類別 | 線性模型會當成距離 | 低 |
| One-hot | 低基數、線性／距離模型 | 欄爆炸 | 低（仍要先切） |
| Count | 「常不常見」本身有意義 | 稀有值不穩 | 計數只看訓練集 |
| Target | 高基數、與目標有穩定關係 | 最容易洩漏 | **只能在訓練摺內算** |
