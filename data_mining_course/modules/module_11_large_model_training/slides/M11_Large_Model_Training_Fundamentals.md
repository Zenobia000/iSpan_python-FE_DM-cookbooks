...
AI 可以填 Trainer／LoRA 設定。人必須對上「資料形狀 → 能接哪類模型」，不要把最小 demo 當成已上線的大模型訓練課。

**留下的那句話：** 資料整理好之後，才問可以微調什麼；LoRA 是參數高效微調，不是魔法。

> 銜接 Module 9 之後回答：**資料整理好之後，可以訓練／微調什麼模型？**
> 定位：**概念藍圖 + CPU 友善的最小 demo**（真實資料）。
